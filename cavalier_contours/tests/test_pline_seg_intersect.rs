...
#[test]
fn arc_arc_two_circle_intersections_only_one_in_sweeps_flipped_roles() {
    // Parameter-role flipped counterpart for the same two-circle/one-sweep
    // branch.
    let v1 = PlineVertex::new(1.0, 1.0, 1.0);
    let v2 = PlineVertex::new(3.0, 1.0, 0.0);
    let u1 = PlineVertex::new(1.5, 1.0, 1.0);
...
#[test]
fn cpp_pline_seg_arc_arc_partial_overlap_same_order_role_flip_order_parity() {
    // Source-aligned with old C++ `intrPlineSegs` coincident-arc overlap branch:
    // for same-direction partial-overlap, role inversion keeps overlap endpoint ordering.
    let v1 = PlineVertex::new(1.0, 1.0, 1.0);
    let v2 = PlineVertex::new(3.0, 1.0, 0.0);
    let u1 = PlineVertex::new(2.0, 0.0, 1.0);
    let u2 = PlineVertex::new(2.0, 2.0, 0.0);
    ...
    assert!(ab_p1.fuzzy_eq(Vector2::new(2.0, 0.0)));
    assert!(ab_p2.fuzzy_eq(Vector2::new(3.0, 1.0)));
...
#[test]
fn cpp_pline_seg_arc_arc_partial_overlap_arc1_reverse_dir_role_flip_order_parity() {
    // Source-aligned with old C++ `intrPlineSegs` coincident-arc overlap branch:
    // when arc1 is reversed, role inversion swaps overlap endpoint ordering.
    let v1 = PlineVertex::new(3.0, 1.0, -1.0);
    let v2 = PlineVertex::new(1.0, 1.0, 0.0);
    let u1 = PlineVertex::new(2.0, 0.0, 1.0);
    let u2 = PlineVertex::new(2.0, 2.0, 0.0);
